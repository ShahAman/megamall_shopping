...
class Control
{
	...
	#Register
	public function reg_cust($cust_name, $cust_email, $cust_pass, $cust_phone, $cust_gender)
	{
		$query = $this->conn->prepare("
			INSERT INTO `customer` (`cust_name`, `cust_email`, `cust_pass`, `cust_phone`, `cust_gender`) VALUES
			(:CUST_NAME, :CUST_EMAIL, :CUST_PASS, :CUST_PHONE, :CUST_GENDER);
		");
	...
	{
		$query = $this->conn->prepare("
			SELECT * FROM `customer` WHERE `cust_email`=:EMID AND `cust_pass`=:PWD
		");
   ...
   #Insert Category Data
	public function add_to_cart($cust_id, $prod_id, $prod_qty)
	{
		$query = $this->conn->prepare("
			INSERT INTO `shopcart` (`cust_id`, `prod_id`, `prod_qty`) VALUES
			(:CUST_ID, :PROD_ID, :PROD_QTY);
		");
	...
	# GET DATA OF SHOPCART
	public function get_cartdata($cust_id)
	{
		$query = $this->conn->prepare("
			SELECT shopcart.*, product.prod_id, product.prod_name, product.prod_pic_1, product.prod_price
			FROM shopcart INNER JOIN product
			ON shopcart.prod_id = product.prod_id
			WHERE shopcart.cust_id = '$cust_id'
		");
	...
	public function update_cart($cart_id, $prod_qty)
	{
		$query = $this->conn->prepare("
			UPDATE `shopcart` SET
			`prod_qty`='$prod_qty'
	...
	public function search_prod($prod_name){
		...
		$dataList = $query->fetchAll(PDO::FETCH_ASSOC);
		$searchrows = $query->rowCount();
		if($searchrows > 0)
			return array("status" => true, "data" => $dataList);
		else 
			return array("status" => false);
	}
	public function get_cartcount($cust_id){
		$query = $this->conn->prepare("
			SELECT COUNT(`prod_qty`) AS TOTAL_QTY FROM `shopcart` WHERE `cust_id` = '$cust_id'
		");
	...
	public function get_cart_summary($cust_id)
	{
		$query = $this->conn->prepare("
			SELECT SUM(product.prod_price * shopcart.prod_qty) AS TOTAL_PRICE
			FROM product INNER JOIN shopcart
			ON product.prod_id = shopcart.prod_id
			WHERE shopcart.cust_id = '$cust_id'
		");
